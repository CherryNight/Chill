#define CHAINWHOOSH			list('modular_twilight_axis/witcher_chain/chainwoosh.ogg','modular_twilight_axis/witcher_chain/chainhit.ogg')
//Интенты
...
/datum/intent/whip/chain_lash
	...
	hitsound = 'modular_twilight_axis/witcher_chain/chainhit.ogg'
	chargetime = 0
	recovery = 7
	penfactor = PEN_LIGHT
	reach = 3
...
/obj/item/rogueweapon/whip/witcher_chain
	...
	var/obj/item/rogueweapon/whip/witcher_chain/transform_to = /obj/item/rogueweapon/whip/witcher_chain
...
/obj/item/rogueweapon/whip/witcher_chain/silver
	...
	grid_width = 32
	grid_height = 64
	transform_to = /obj/item/rogueweapon/whip/witcher_chain/silver
...
/obj/item/clothing/wrists/roguetown/bracers/witcher
	...
	var/obj/item/rogueweapon/whip/witcher_chain/transform_whip
...
/obj/item/clothing/wrists/roguetown/bracers/witcher/attack_right(mob/user)
	if(do_after(user,3 SECONDS, TRUE, src, allow_movement = TRUE))
		var/obj/item/rogueweapon/whip/witcher_chain/W = new transform_to(get_turf(src))
...
/obj/item/rogueweapon/whip/witcher_chain/attack_right(mob/user)
	if(do_after(user,3 SECONDS, TRUE, src, allow_movement = TRUE))
		var/obj/item/clothing/wrists/roguetown/bracers/witcher/B = new transform_to(get_turf(src))
		B.obj_integrity = obj_integrity
		B.stored_blade_int = blade_int
		B.transform_whip = src
		if(user.is_holding(src))
			user.dropItemToGround(src)
			user.put_in_hands(B)
		qdel(src)
...
/datum/special_intent/witcher_chain_hook
	name = "Chain Ensnare"
	desc = "Вы бросаете утяжеленный наконечник цепи в сторону цели. Наконечник пробивает броню, обвивает цель и силой втягивает её в ближний бой для завершающего удара."
	use_clickloc = TRUE
...
/datum/special_intent/witcher_chain_hook/process_attack()
	...
	if(!(howner.mobility_flags & MOBILITY_STAND))
		to_chat(howner, span_warning("Нужно твердо стоять на ногах, чтобы метнуть цепь!"))
		return
	...
	playsound(howner, 'modular_twilight_axis/witcher_chain/chainwoosh.ogg', 80, TRUE)
...
/datum/special_intent/witcher_chain_hook/proc/on_hook_target()
	...
	if(!istype(hooked_target,/mob/living/simple_animal))
		...
		if(!(hooked_target.apply_damage(25, BRUTE, BODY_ZONE_CHEST, armor_block)))
			hooked_target.visible_message(span_warning("Наконечник отскакивает от брони [hooked_target], не в силах зацепиться!"))
			begin_return()
			return

	hooked_target.visible_message(
		span_warning("[hooked_target] пробит наконечником цепи и захвачен кольцами!"),
		span_userdanger("Острый наконечник цепи пробивает меня, звенья захлестывают тело!")
	)
	...
	playsound(target_turf, 'modular_twilight_axis/witcher_chain/chainhit.ogg', 90, TRUE)
...
/datum/special_intent/witcher_chain_hook/proc/finish_reel()
	...
	if(hooked_target && !QDELETED(hooked_target) && get_dist(hooked_target, howner) <= 1)
		...
		hooked_target.visible_message(
			span_danger("[howner] с силой дергает цепь, сбивая [hooked_target] с ног сокрушительным ударом!"),
			span_userdanger("[howner] дергает цепь на себя и сбивает меня с ног!")
		)
